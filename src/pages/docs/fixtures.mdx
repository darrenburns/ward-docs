...
sortKey: 3
---
import {TestOutputLine, TerminalExample} from "../../components/ward-test-output"
...
They provide a modular, composable alternative to `setup` and `teardown` methods that appear in many test frameworks.
...
By directly binding the fixture as a default argument to our test function, we've told Ward to resolve the
fixture and inject it into our test. Inside our test, the variable `expected_user` is the object `User(id=1, name="sam")`.

### Real-world examples

<CommitExample hash={"097348834b444cb55c7505aa41aa68e4ed09996c"} 
  url={"https://github.com/darrenburns/ward/commit/097348834b444cb55c7505aa41aa68e4ed09996c"}
  title={"Extracting common setup code into a fixture"} 
  repo={"darrenburns/ward"}
  description={"This is an example of a commit from the Ward repo where setup code common between multiple\
  tests was extracted into a fixture, which was then shared between the tests."} />


## The `@using` decorator

An alternative approach to injecting fixtures into tests is the
`@using` decorator.

This approach lets us use positional arguments in our test signature, and 
declare which fixture each argument refers to using the decorator.
...
In the example above, we tell Ward to bind the resolved value of the 
`user` fixture to the `expected_user` position argument.

There are two main reasons to use `@using` over default arguments:

1. You prefer the approach/API.
2. You want to use something like [Hypothesis](https://hypothesis.readthedocs.io), which asserts 
that your tests cannot have default arguments.
...
To solve this problem, Ward lets you *scope* your fixtures. The scope of a fixture determines how long it is cached for.
...
the isolated nature of tests, and introduces hidden dependencies between them.
Ward will warn you if it detects a `global` or `module` scoped fixture has been mutated inside a test (coming in v1.0).
</Warning>
...
You can inject a fixture into another fixture in the same way that you'd inject it into a test:
by binding it as a default argument.
...
<TerminalExample>
    <TestOutputLine marker="PASS" moduleName="test_composition" lineNumber="14" description="fixtures can be composed"/>
</TerminalExample>
...
Notice how we `yield` the value of the fixture in the test below. Ward will inject the yielded value into the 
test, and after the test has run, all code below the `yield` will be executed.
