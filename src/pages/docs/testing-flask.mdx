---
title: "Testing a Flask application"
path: "/guide/testing-flask"
section: "tutorials"
sortKey: 1
---
import {TerminalCommand} from "../../components/ward-test-output"
...
def _(client=test_client):
    ...
    expect(res.data).equals("The user is alice")
```
...
Great, our test passed! The fully green bar indicates a 100% success rate!
...
def _(client=test_client):
    res = client.get("/users/alice")
    expect(res.data).equals(b"The user is alice")
```

If we run our tests again, we see that they both pass!
